...
class AudioLanguageModelTrainingWrapper(pl.LightningModule):
    def __init__(
        ...
        if use_ema:
            self.model_ema = EMA(
                self.model, ema_model=ema_copy, beta=0.99, update_every=10
            )
        ...
        if optimizer_configs is None:
            optimizer_configs = {
                "lm": {
                    "optimizer": {
                        "type": "AdamW",
                        "config": {"lr": lr, "betas": (0.9, 0.95), "weight_decay": 0.1},
                    }
                }
            }
        else:
    ...
    def configure_optimizers(self):
        ...
        opt_lm = create_optimizer_from_config(
            lm_opt_config["optimizer"], self.model.parameters()
        )
    ...
    def training_step(self, batch, batch_idx):
        ...
        padding_masks = torch.stack(padding_masks, dim=0).to(
            self.device
        )  # Shape (batch_size, sequence_length)

        # Interpolate padding masks to the same length as the codes
        padding_masks = F.interpolate(
            padding_masks.unsqueeze(1).float(), size=codes.shape[2], mode="nearest"
        ).bool()
        ...
        lm_output = self.model.compute_logits(
            codes, condition_tensors=condition_tensors, cfg_dropout_prob=0.1
        )
        ...
        cross_entropy, cross_entropy_per_codebook = self._compute_cross_entropy(
            logits, codes, logits_mask
        )
...
class AudioLanguageModelDemoCallback(pl.Callback):
    ...
    def on_train_batch_end(
        self,
        trainer,
        module: AudioLanguageModelTrainingWrapper,
        outputs,
        batch,
        batch_idx,
    ):
        ...
        demo_length_tokens = (
            self.demo_samples // module.model.pretransform.downsampling_ratio
        )
        ...
        try:
            ...
            for cfg_scale in self.demo_cfg_scales:
                ...
                model = (
                    module.model
                )  # module.model_ema.ema_model if module.model_ema is not None else module.model
