...
@torch.no_grad()
def sample_flow_dpmpp_w_intermediates(
    model, x, sigmas=None, steps=None, callback=None, dist_shift=None, **extra_args
):
    ...
    for i in range(len(t) - 1):
        ...
        if callback is not None:
            callback(
                {"x": x, "i": i, "sigma": t[i], "sigma_hat": t[i], "denoised": denoised}
            )
...
class ARCTrainingWrapper(pl.LightningModule):
    ...
    def __init__(
        ...
        timestep_sampler: tp.Literal[
            "uniform", "logit_normal", "trunc_logit_normal"
        ] = "uniform",
        ...
        self.discriminator_dit_layer = self.discriminator_config.get(
            "dit_hidden_layer", None
        )

        self.do_contrastive_disc = self.discriminator_config.get("contrastive", False)

        self.include_grad_penalties = self.discriminator_config.get(
            "include_grad_penalties", False
        )

        assert (
            self.discriminator_dit_layer is not None
        ), "Must specify discriminator dit_hidden_layer in ARC config"
        ...
        if discriminator_type == "convnext":
            ...
            self.discriminator_head = ConvNeXtDiscriminator(
                in_channels=self.discriminator.model.model.transformer.dim,
                latent_dim=1,
            ...
            self.discriminator_head = ConvDiscriminator(
                channels=self.discriminator.model.model.transformer.dim,
                **discriminator_model_config,
            )

        self.gen_gan_weight = self.discriminator_config.get("weights", {}).get(
            "generator", 1.0
        )
        self.dis_gan_weight = self.discriminator_config.get("weights", {}).get(
            "discriminator", 1.0
        )

        if self.do_contrastive_disc:
            self.contrastive_loss_weight = self.discriminator_config.get(
                "weights", {}
            ).get("contrastive", 1.0)
        ...
        assert (
            optimizer_configs is not None
        ), "Must specify optimizer_configs in training config"
        ...
        self.model_last_layer = (
            self.diffusion.model.model.transformer.project_out.weight
        )
    ...
    def configure_optimizers(self):
        ...
        opt_disc = create_optimizer_from_config(
            disc_opt_config["optimizer"],
            list(self.discriminator_head.parameters())
        ...
        if "scheduler" in diffusion_opt_config and "scheduler" in disc_opt_config:
            ...
            sched_disc = create_scheduler_from_config(
                disc_opt_config["scheduler"], opt_disc
            )
            sched_disc_config = {"scheduler": sched_disc, "interval": "step"}
    ...
    def ode_warmup_step(self, diffusion_input, metadata, padding_masks):
        ...
        if refresh_diff_states:
            ...
            self.diff_states = sample_flow_dpmpp_w_intermediates(
                ...
                dist_shift=self.teacher_model.dist_shift,
                batch_cfg=True,
            )
        ...
        x_t = (
            self.diff_states["x"]
            .clone()
            .detach()[ixs.flatten(), torch.arange(diffusion_input.shape[0])]
            .squeeze(0)
        )
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.global_step < self.ode_warmup_steps:
            ode_mse_loss = self.ode_warmup_step(
                diffusion_input, metadata, padding_masks
            )
            ...
            if self.clip_grad_norm > 0.0:
                torch.nn.utils.clip_grad_norm_(
                    self.diffusion.parameters(), self.clip_grad_norm
                )
            opt_gen.step()
        ...
        if train_gen or self.global_step < self.ode_warmup_steps:
            ...
            with torch.no_grad():
                v_t_student = checkpoint(
                    self.diffusion, x_t, t, cond=conditioning
                ).detach()
        ...
        if train_gen:
            ...
            x_t_gan = (
                denoised_student * (1 - t_gan)[:, None, None]
                + noise * t_gan[:, None, None]
            )
            ...
            # Relativistic discriminator loss
            x_t_gan_real = (
                diffusion_input * (1 - t_gan)[:, None, None]
                + noise * t_gan[:, None, None]
            )
            ...
            disc_scores_real = self.discriminator_head(
                v_t_gan_real_hidden_states.transpose(1, 2)
            )
            ...
            reals_gan_hidden_states = checkpoint(
                self.discriminator,
                reals_t_gan,
                t_gan,
                cond=fake_conditioning,
            ...
            denoised_gan_hidden_states = checkpoint(
                self.discriminator,
                denoised_t_gan,
                t_gan,
                cond=fake_conditioning,
            ...
            disc_scores_reals = checkpoint(
                self.discriminator_head, reals_gan_hidden_states
            )
            disc_scores_denoised = checkpoint(
                self.discriminator_head, denoised_gan_hidden_states
            )
            ...
            if self.include_grad_penalties:
                ...
                noised_reals_t_gan = (
                    reals_t_gan + r1_approx_variance * torch.randn_like(reals_t_gan)
                )
                noised_denoised_t_gan = (
                    denoised_t_gan
                    + r1_approx_variance * torch.randn_like(denoised_t_gan)
                )
                noised_reals_gan_hidden_states = checkpoint(
                    self.discriminator,
                    noised_reals_t_gan,
                    t_gan,
                    cond=fake_conditioning,
                ...
                noised_denoised_gan_hidden_states = checkpoint(
                    self.discriminator,
                    noised_denoised_t_gan,
                    t_gan,
                    cond=fake_conditioning,
                ...
                disc_scores_noised_reals = checkpoint(
                    self.discriminator_head, noised_reals_gan_hidden_states
                )
                disc_scores_noised_denoised = checkpoint(
                    self.discriminator_head, noised_denoised_gan_hidden_states
                )
            ...
            if self.do_contrastive_disc:
                ...
                for i in range(reals.shape[0]):
                    ...
                    for rolled_key in rolled_keys:
                        rolled_metadata[i][rolled_key] = metadata[
                            (i + 1) % reals.shape[0]
                        ][rolled_key]

                rolled_conditioning = self.discriminator.conditioner(
                    rolled_metadata, self.device
                )
                ...
                rolled_reals_gan_hidden_states = checkpoint(
                    ...
                    reals_t_gan,
                    t_gan,
                    cond=rolled_conditioning,
                    cfg_scale=1.0,
                    exit_layer_ix=self.discriminator_dit_layer,
                ).transpose(1, 2)

                disc_scores_rolled_reals = checkpoint(
                    self.discriminator_head, rolled_reals_gan_hidden_states
                )

                contrastive_loss_dis = (
                    self.calculate_disc_loss(
                        disc_scores_reals, disc_scores_rolled_reals
                    )
    ...
    def build_noise_dist(self, key):
        ...
        match dist:
            ...
            case "trunc_logit_normal":
                return lambda b: 1 - truncated_logistic_normal_rescaled(b).to(
                    self.device
                )
            case "one_shot":
            ...
            case "logsnr_uniform":
                ...
                return lambda b: torch.sigmoid(
                    -(
                        torch.rand(b, device=self.device) * (max_logsnr - min_logsnr)
                        + min_logsnr
                    )
                )
            case _:
